...
class GeneratorPretrainingGenerator(Sequence):
    ...
    def __init__(self, path, B, T=40, min_count=1, shuffle=True):
        ...
        self.EOS = 2
        self.UNK = 3
        self.PAD_TOKEN = '<PAD>'    # 长度不够的填充
        self.UNK_TOKEN = '<UNK>'    # 词典外的词填充
        self.BOS_TOKEN = '<S>'      # 开始
        self.EOS_TOKEN = '</S>'     #介绍
        self.path = path
        self.B = B
        self.T = T
        self.min_count = min_count  # 记录在词汇表中的最小出现次数
        ...
        self.vocab.build_vocab(sentences, self.min_count)       #遍历训练集中的所有元素，检查sentences字段的内容，并将其添加到其词汇表中。

        self.word2id = self.vocab.word2id       # word到id的映射
        self.id2word = self.vocab.id2word       # id到word的映射
